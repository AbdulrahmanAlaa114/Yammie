...
public extension Reactive where Base: UITextView {
    ...
    var didBeginEditing: ControlEvent<Void> {
        return ControlEvent<Void>(
            events: delegate
                .methodInvoked(#selector(UITextViewDelegate.textViewDidBeginEditing(_:)))
                .map { _ in
                    ()
                }
        )
    ...
    var didChange: ControlEvent<Void> {
        return ControlEvent<Void>(
            events: delegate.methodInvoked(#selector(UITextViewDelegate.textViewDidChange(_:)))
                .map { _ in
                    ()
                }
        )
    ...
    var didChangeSelection: ControlEvent<Void> {
        return ControlEvent<Void>(
            events: delegate
                .methodInvoked(#selector(UITextViewDelegate.textViewDidChangeSelection(_:)))
                .map { _ in
                    ()
                }
        )
