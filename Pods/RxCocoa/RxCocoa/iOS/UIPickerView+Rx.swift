...
public extension Reactive where Base: UIPickerView {
    ...
    -> Disposable {
        return RxPickerViewDelegateProxy.installForwardDelegate(
            delegate,
    ...
    func modelSelected<T>(_: T.Type) -> ControlEvent<[T]> {
        let source = itemSelected
            .flatMap { [weak view = self.base as UIPickerView] _, component -> Observable<[T]> in
        ...
        -> Disposable where Source.Element == Sequence
    {
        return { attributedTitleForRow in
            let adapter =
                RxAttributedStringPickerViewAdapter<Sequence>(attributedTitleForRow: attributedTitleForRow)
            return self.items(adapter: adapter)(source)
        }
    }
    ...
    func items<
        ...
        Adapter: RxPickerViewDataSourceType & UIPickerViewDataSource &
        ...
        -> Disposable where Source.Element == Adapter.Element
    {
        return { source in
            ...
            let dataSourceSubscription = source.subscribeProxyDataSource(
                ofObject: self.base,
                dataSource: adapter,
                retainDataSource: true,
                binding: { [weak pickerView = self.base] (_: RxPickerViewDataSourceProxy, event) in
    ...
    func model<T>(at indexPath: IndexPath) throws -> T {
        let dataSource: SectionedViewDataSourceType = castOrFatalError(
            ...
            message: "This method only works in case one of the `rx.itemTitles, rx.itemAttributedTitles, items(_ source: O)` methods was used."
        )
