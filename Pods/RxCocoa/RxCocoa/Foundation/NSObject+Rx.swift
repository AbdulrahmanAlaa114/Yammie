...
public extension Reactive where Base: AnyObject {
    ...
    var deallocated: Observable<Void> {
        return synchronized {
            if let deallocObservable = objc_getAssociatedObject(
                self.base,
                &deallocatedSubjectContext
            ) as? DeallocObservable {
    ...
    Disposable
{
    ...
    init(parent: KVOObservableProtocol, callback: @escaping Callback) {
        ...
        super.init(
            ...
            retainTarget: parent.retainTarget,
            keyPath: parent.keyPath,
            options: parent.options.nsOptions,
            callback: callback
        )
...
private func observeWeaklyKeyPathFor(
    ...
    let propertyObservable = KVOObservable(
        object: target,
        keyPath: propertyName,
        options: options.union(.initial),
        retainTarget: false
    ) as KVOObservable<AnyObject>
    ...
    return propertyObservable
        .flatMapLatest { (nextTarget: AnyObject?) -> Observable<AnyObject?> in
            ...
            if nextObject == nil {
                return Observable.error(RxCocoaError.invalidObjectOnKeyPath(
                    object: nextTarget!,
                    sourceObject: strongTarget ?? NSNull(),
