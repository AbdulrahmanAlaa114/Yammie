...
public extension Reactive where Base: UITableView {
        ...
        where DataSource.Element == Source.Element
    {
        return { source in
            ...
            ) { [weak tableView = self.base] (_: RxTableViewDataSourceProxy, event) in
...
public extension Reactive where Base: UITableView {
    ...
    var itemAccessoryButtonTapped: ControlEvent<IndexPath> {
        let source: Observable<IndexPath> = delegate
            .methodInvoked(#selector(UITableViewDelegate.tableView(_:accessoryButtonTappedForRowWith:)))
            .map { a in
                try castOrThrow(IndexPath.self, a[1])
            }
    ...
    var itemMoved: ControlEvent<ItemMovedEvent> {
        let source: Observable<ItemMovedEvent> = dataSource
            .methodInvoked(#selector(UITableViewDataSource.tableView(_:moveRowAt:to:)))
            .map { a in
    ...
    var willDisplayCell: ControlEvent<WillDisplayCellEvent> {
        let source: Observable<WillDisplayCellEvent> = delegate
    ...
    var didEndDisplayingCell: ControlEvent<DidEndDisplayingCellEvent> {
        let source: Observable<DidEndDisplayingCellEvent> = delegate
            .methodInvoked(#selector(UITableViewDelegate.tableView(_:didEndDisplaying:forRowAt:)))
            .map { a in
                try (castOrThrow(UITableViewCell.self, a[1]), castOrThrow(IndexPath.self, a[2]))
            }
    ...
    func modelDeselected<T>(_: T.Type) -> ControlEvent<T> {
        let source: Observable<T> = itemDeselected
            .flatMap { [weak view = self.base as UITableView] indexPath -> Observable<T> in
                guard let view = view else {
                    return Observable.empty()
                }
    ...
    func modelDeleted<T>(_: T.Type) -> ControlEvent<T> {
        let source: Observable<T> = itemDeleted
            .flatMap { [weak view = self.base as UITableView] indexPath -> Observable<T> in
                guard let view = view else {
                    return Observable.empty()
                }
...
public extension Reactive where Base: UITableView {
    ...
    -> Disposable {
        return RxTableViewDataSourcePrefetchingProxy.installForwardDelegate(
            prefetchDataSource,
    ...
    var cancelPrefetchingForRows: ControlEvent<[IndexPath]> {
        let source = prefetchDataSource
            .methodInvoked(#selector(UITableViewDataSourcePrefetching.tableView(_:cancelPrefetchingForRowsAt:)))
            .map { a in
                try castOrThrow([IndexPath].self, a[1])
            }
...
public extension Reactive where Base: UITableView {
     ...
     */
    var didUpdateFocusInContextWithAnimationCoordinator: ControlEvent<(
        context: UITableViewFocusUpdateContext,
        animationCoordinator: UIFocusAnimationCoordinator
    )> {
