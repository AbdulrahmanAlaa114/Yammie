    ...
    CustomDebugStringConvertible
{
    ...
    case objectMessagesAlreadyBeingIntercepted(
        target: AnyObject,
        interceptionMechanism: RxCocoaInterceptionMechanism
    )
    ...
    /// is because in some cases intercepting sent messages requires replacing implementation with
...
public extension RxCocoaObjCRuntimeError {
    ...
    var debugDescription: String {
        ...
        case let .objectMessagesAlreadyBeingIntercepted(target, interceptionMechanism):
            let interceptionMechanismDescription = interceptionMechanism == .kvo ? "KVO" :
                "other interception mechanism"
    ...
    func rxCocoaErrorForTarget(_ target: AnyObject) -> RxCocoaObjCRuntimeError {
        ...
        if error.domain == RXObjCRuntimeErrorDomain {
            ...
            case .objectMessagesAlreadyBeingIntercepted:
                ...
                return .objectMessagesAlreadyBeingIntercepted(
                    target: target,
                    interceptionMechanism: isKVO ? .kvo : .unknown
                )
