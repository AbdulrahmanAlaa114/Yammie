        ...
        -> Disposable where DataSource.Element == Source.Element
    {
        return { source in
            ...
            return source
                ...
                ) { [weak collectionView = self.base] (_: RxCollectionViewDataSourceProxy, event) in
...
public extension Reactive where Base: UICollectionView {
    ...
    typealias DisplayCollectionViewSupplementaryViewEvent = (
    ...
    var willDisplayCell: ControlEvent<DisplayCollectionViewCellEvent> {
        let source: Observable<DisplayCollectionViewCellEvent> = delegate
            .methodInvoked(#selector(UICollectionViewDelegate.collectionView(_:willDisplay:forItemAt:)))
            .map { a in
    ...
    var willDisplaySupplementaryView: ControlEvent<DisplayCollectionViewSupplementaryViewEvent> {
        let source: Observable<DisplayCollectionViewSupplementaryViewEvent> = delegate
    ...
    var didEndDisplayingCell: ControlEvent<DisplayCollectionViewCellEvent> {
        let source: Observable<DisplayCollectionViewCellEvent> = delegate
            .methodInvoked(#selector(UICollectionViewDelegate.collectionView(_:didEndDisplaying:forItemAt:)))
            .map { a in
                try (castOrThrow(UICollectionViewCell.self, a[1]), castOrThrow(IndexPath.self, a[2]))
            }
    ...
    func model<T>(at indexPath: IndexPath) throws -> T {
        let dataSource: SectionedViewDataSourceType = castOrFatalError(
            ...
            message: "This method only works in case one of the `rx.itemsWith*` methods was used."
        )
...
public extension Reactive where Base: UICollectionView {
    ...
    -> Disposable {
        return RxCollectionViewDataSourcePrefetchingProxy.installForwardDelegate(
            prefetchDataSource,
            retainDelegate: false,
            onProxyForObject: base
        )
...
public extension Reactive where Base: UICollectionView {
    ...
    var didUpdateFocusInContextWithAnimationCoordinator: ControlEvent<(
        ...
        let source = delegate
            .methodInvoked(#selector(UICollectionViewDelegate.collectionView(_:didUpdateFocusIn:with:)))
