...
public extension PrimitiveSequenceType where Trait == SingleTrait {
    ...
    where Collection.Element == PrimitiveSequence<Trait, Element> {
     ...
     */
    static func zip<Collection: Swift.Collection>(_ collection: Collection) -> PrimitiveSequence<Trait, [Element]>
    where Collection.Element == PrimitiveSequence<Trait, Element> {
        if collection.isEmpty {
