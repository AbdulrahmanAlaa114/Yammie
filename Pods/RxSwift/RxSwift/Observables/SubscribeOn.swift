...
private final class SubscribeOnSink<Ob: ObservableType, Observer: ObserverType>: Sink<Observer>,
ObserverType where Ob.Element == Observer.Element {
    ...
    func run() -> Disposable {
        ...
        let disposeSchedule = parent.scheduler.schedule(()) { _ -> Disposable in
            ...
            disposeEverything.disposable = ScheduledDisposable(
...
private final class SubscribeOn<Ob: ObservableType>: Producer<Ob.Element> {
    ...
    override func run<Observer: ObserverType>(
        _ observer: Observer,
        cancel: Cancelable
    ) -> (sink: Disposable, subscription: Disposable) where Observer.Element == Ob.Element {
