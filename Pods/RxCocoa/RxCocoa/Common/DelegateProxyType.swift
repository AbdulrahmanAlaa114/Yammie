...
public extension DelegateProxyType {
    ...
    /// When make 'Rx*DelegateProxy' subclass, call 'Rx*DelegateProxySubclass.register(for:_)' 1 time, or use it in
    ...
    /// self.delegate.observe(#selector(UISearchBarDelegate.searchBar(_:textDidChange:)))
    ///             ...
    ///         }
    ///     }
    ...
    /// Sets forward delegate for `DelegateProxyType` associated with a specific object and return disposable that can
    /// be used to unset the forward to delegate.
    /// Using this method will also make sure that potential original object cached selectors are cleared and will
    /// report any accidental forward delegate mutations.
    ///
    ...
    static func installForwardDelegate(
...
@available(iOS 10.0, tvOS 10.0, *)
public extension DelegateProxyType where ParentObject: HasPrefetchDataSource,
Self.Delegate == ParentObject.PrefetchDataSource {
    static func currentDelegate(for object: ParentObject) -> Delegate? {
...
private class DelegateProxyFactory {
    ...
    fileprivate func createProxy(for object: AnyObject) -> AnyObject {
        ...
        rxFatalError(
            "DelegateProxy has no factory of \(object). Implement DelegateProxy subclass for \(object) first."
        )
    }
}
