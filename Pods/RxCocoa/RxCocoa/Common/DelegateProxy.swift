...
open class DelegateProxy<P: AnyObject, D>: _RXDelegateProxy {
    ...
    public init<Proxy: DelegateProxyType>(parentObject: ParentObject, delegateProxy: Proxy.Type)
        where Proxy: DelegateProxy<ParentObject, Delegate>, Proxy.ParentObject == ParentObject,
    ...
    fileprivate func checkSelectorIsObservable(_ selector: Selector) {
        ...
        if hasWiredImplementation(for: selector) {
            print(
                "⚠️ Delegate proxy is already implementing `\(selector)`, a more performant way of registering might exist."
            )
            return
        }
    ...
    open func setForwardToDelegate(_ delegate: Delegate?, retainDelegate: Bool) {
        ...
        let sentSelectors: [Selector] = _sentMessageForSelector.values.filter { $0.hasObservers }
            .map { $0.selector }
        let invokedSelectors: [Selector] = _methodInvokedForSelector.values.filter { $0.hasObservers }
            .map { $0.selector }
...
private final class MessageDispatcher {
    ...
    init<P, D>(selector: Selector, delegateProxy _delegateProxy: DelegateProxy<P, D>) {
        ...
        self.result = dispatcher
            .do(
                onSubscribed: { weakDelegateProxy?.checkSelectorIsObservable(selector); weakDelegateProxy?.reset()
                },
